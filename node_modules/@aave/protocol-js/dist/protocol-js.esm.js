...
function getCompoundedBorrowBalance(reserve, userReserve, currentTimestamp) {
  ...
  if (userReserve.borrowRateMode === BorrowRateMode.Variable) {
    var compoundedInterest = calculateCompoundedInterest(reserve.variableBorrowRate, currentTimestamp, reserve.lastUpdateTimestamp);
    cumulatedInterest = rayDiv(rayMul(compoundedInterest, reserve.variableBorrowIndex), userReserve.variableBorrowIndex);
  } else {
    // if stable
    cumulatedInterest = calculateCompoundedInterest(userReserve.borrowRate, currentTimestamp, userReserve.lastUpdateTimestamp);
  }

  var borrowBalanceRay = wadToRay(principalBorrows);
  return rayToWad(rayMul(borrowBalanceRay, cumulatedInterest));
}
var calculateCompoundedInterest = function calculateCompoundedInterest(rate, currentTimestamp, lastUpdateTimestamp) {
...
function calculateHealthFactorFromBalances(collateralBalanceETH, borrowBalanceETH, totalFeesETH, currentLiquidationThreshold) {
...
function calculateHealthFactorFromBalancesBigUnits(collateralBalanceETH, borrowBalanceETH, totalFeesETH, currentLiquidationThreshold) {
  return calculateHealthFactorFromBalances(collateralBalanceETH, borrowBalanceETH, totalFeesETH, new BigNumber(currentLiquidationThreshold).multipliedBy(100).decimalPlaces(0, BigNumber.ROUND_DOWN));
}
...
function getReserveNormalizedIncome(reserve, currentTimestamp) {
  var liquidityRate = reserve.liquidityRate,
  ...
  var cumulatedInterest = calculateLinearInterest(liquidityRate, currentTimestamp, lastUpdateTimestamp);
...
function calculateCumulatedBalance(balance, userReserve, poolReserve, currentTimestamp) {
  return rayToWad(rayDiv(rayMul(wadToRay(balance), getReserveNormalizedIncome(poolReserve, currentTimestamp)), userReserve.userBalanceIndex));
}
...
function computeRawUserSummaryData(poolReservesData, rawUserReserves, userId, usdPriceEth, currentTimestamp) {
  ...
  var healthFactor = calculateHealthFactorFromBalances(totalCollateralETH, totalBorrowsETH, totalFeesETH, currentLiquidationThreshold);
  ...
  var availableBorrowsETH = calculateAvailableBorrowsETH(totalCollateralETH, totalBorrowsETH, totalFeesETH, currentLtv);
...
function calculateAverageRate(index0, index1, timestamp0, timestamp1) {
  return new BigNumber(index1).dividedBy(index0).minus('1').dividedBy(timestamp1 - timestamp0).multipliedBy('31536000').toString();
}
...
var index = {
  ...
  calculateHealthFactorFromBalancesBigUnits: calculateHealthFactorFromBalancesBigUnits,
  calculateAvailableBorrowsETH: calculateAvailableBorrowsETH,
  ...
  calculateAverageRate: calculateAverageRate,
...
var LTV_PRECISION = 4;
function calculateCompoundedInterest$1(rate, currentTimestamp, lastUpdateTimestamp) {
  var timeDelta = valueToZDBigNumber(currentTimestamp - lastUpdateTimestamp);
  var ratePerSecond = valueToZDBigNumber(rate).dividedBy(SECONDS_PER_YEAR);
  return binomialApproximatedRayPow(ratePerSecond, timeDelta);
}
function getCompoundedBalance(_principalBalance, _reserveIndex, _reserveRate, _lastUpdateTimestamp, currentTimestamp) {
  ...
  var compoundedInterest = calculateCompoundedInterest$1(_reserveRate, currentTimestamp, _lastUpdateTimestamp);
...
function getCompoundedStableBalance(_principalBalance, _userStableRate, _lastUpdateTimestamp, currentTimestamp) {
  ...
  var cumulatedInterest = calculateCompoundedInterest$1(_userStableRate, currentTimestamp, _lastUpdateTimestamp);
  var principalBalanceRay = wadToRay(principalBalance);
  return rayToWad(rayMul(principalBalanceRay, cumulatedInterest));
}
function calculateHealthFactorFromBalances$1(collateralBalanceETH, borrowBalanceETH, currentLiquidationThreshold) {
  if (valueToBigNumber(borrowBalanceETH).eq(0)) {
    return valueToBigNumber('-1'); // invalid number
  }
...
function calculateHealthFactorFromBalancesBigUnits$1(collateralBalanceETH, borrowBalanceETH, currentLiquidationThreshold) {
  return calculateHealthFactorFromBalances$1(collateralBalanceETH, borrowBalanceETH, new BigNumber(currentLiquidationThreshold).multipliedBy(pow10(LTV_PRECISION)).decimalPlaces(0, BigNumber.ROUND_DOWN));
}
function calculateAvailableBorrowsETH$1(collateralBalanceETH, borrowBalanceETH, currentLtv) {
  if (valueToZDBigNumber(currentLtv).eq(0)) {
    return valueToZDBigNumber('0');
  }
...
function calculateAverageRate$1(index0, index1, timestamp0, timestamp1) {
...
function computeUserReserveData$1(poolReserve, userReserve, usdPriceEth, currentTimestamp) {
  var priceInEth = poolReserve.price.priceInEth,
      decimals = poolReserve.decimals;
  var underlyingBalance = getCompoundedBalance(userReserve.scaledATokenBalance, poolReserve.liquidityIndex, poolReserve.liquidityRate, poolReserve.lastUpdateTimestamp, currentTimestamp).toString();
...
function computeRawUserSummaryData$1(poolReservesData, rawUserReserves, userId, usdPriceEth, currentTimestamp) {
  ...
  var healthFactor = calculateHealthFactorFromBalances$1(totalCollateralETH, totalBorrowsETH, currentLiquidationThreshold);
  var totalCollateralUSD = totalCollateralETH.multipliedBy(pow10(USD_DECIMALS)).dividedBy(usdPriceEth).toString();
  var totalLiquidityUSD = totalLiquidityETH.multipliedBy(pow10(USD_DECIMALS)).dividedBy(usdPriceEth).toString();
  var totalBorrowsUSD = totalBorrowsETH.multipliedBy(pow10(USD_DECIMALS)).dividedBy(usdPriceEth).toString();
  var availableBorrowsETH = calculateAvailableBorrowsETH$1(totalCollateralETH, totalBorrowsETH, currentLtv);
  return {
    totalLiquidityUSD: totalLiquidityUSD,
    totalCollateralUSD: totalCollateralUSD,
    totalBorrowsUSD: totalBorrowsUSD,
...
function formatReserves$1(reserves, currentTimestamp, reserveIndexes30DaysAgo) {
  return reserves.map(function (reserve) {
    ...
    var totalVariableDebt = normalize(rayMul(reserve.totalScaledVariableDebt, reserve.variableBorrowIndex), reserve.decimals);
    var totalStableDebt = normalize(rayMul(reserve.totalPrincipalStableDebt, calculateCompoundedInterest$1(reserve.averageStableRate, currentTimestamp, reserve.stableDebtLastUpdateTimestamp)), reserve.decimals);
    ...
    return _extends({}, reserve, {
      ...
      variableBorrowRate: normalize(reserve.variableBorrowRate, RAY_DECIMALS),
      avg30DaysVariableBorrowRate: reserve30DaysAgo ? calculateAverageRate$1(reserve30DaysAgo.variableBorrowIndex, reserve.variableBorrowIndex, reserve30DaysAgo.timestamp, reserve.lastUpdateTimestamp) : undefined,
      avg30DaysLiquidityRate: reserve30DaysAgo ? calculateAverageRate$1(reserve30DaysAgo.liquidityIndex, reserve.liquidityIndex, reserve30DaysAgo.timestamp, reserve.lastUpdateTimestamp) : undefined,
      stableBorrowRate: normalize(reserve.stableBorrowRate, RAY_DECIMALS),
      liquidityRate: normalize(reserve.liquidityRate, RAY_DECIMALS),
      liquidityIndex: normalize(reserve.liquidityIndex, RAY_DECIMALS),
...
(function (Market) {
  Market["Proto"] = "proto"; // Uniswap = 'uniswap',
})(Market || (Market = {}));
...
var _commonContractAddres, _Market$Proto, _distinctContractAddr, _aaveGovernanceV2Addr;
...
}, _Market$Proto), _distinctContractAddr);
...
var enabledNetworksByService = {
  ...
  lendingPool: (_lendingPool = {}, _lendingPool[Market.Proto] = [Network.kovan, Network.mainnet], _lendingPool),
...
function (_BaseService) {
  ...
  function () {
    ...
    runtime_1.mark(function _callee7(_ref7) {
      var liquidator, liquidatedUser, debtReserve, collateralReserve, purchaseAmount, getAToken, txs, _this$erc20Service3, isApproved, approve, getTokenData, approved, approveTx, _ref8, debtReserveInfo, reserveDecimals, convertedAmount, lendingPoolContract, txCallback;

      return runtime_1.wrap(function _callee7$(_context7) {
        while (1) {
          switch (_context7.prev = _context7.next) {
            case 0:
              liquidator = _ref7.liquidator, liquidatedUser = _ref7.liquidatedUser, debtReserve = _ref7.debtReserve, collateralReserve = _ref7.collateralReserve, purchaseAmount = _ref7.purchaseAmount, getAToken = _ref7.getAToken;
              txs = [];
            ...
            case 9:
              ...
              convertedAmount = parseNumber(purchaseAmount, reserveDecimals);
              lendingPoolContract = this.getContractInstance(this.lendingPoolAddress);
              txCallback = this.generateTxCallback({
                rawTxMethod: function rawTxMethod() {
          ...
          while (1) {
            switch (_context.prev = _context.next) {
              case 0:
                ...
                if (!(_this.config.network === Network.ropsten)) {
                  _context.next = 3;
                  break;
                }
              ...
              case 3:
                _this$erc20Service = _this.erc20Service, isApproved = _this$erc20Service.isApproved, approve = _this$erc20Service.approve, decimalsOf = _this$erc20Service.decimalsOf;
                migratorContract = _this.getContractInstance(_this.migratorAddress);
                _context.next = 7;
              ...
              case 7:
                ...
                _context.next = 10;
                return isApproved(lendToken, user, _this.migratorAddress, amount);

              case 10:
                approved = _context.sent;

                if (!approved) {
                  txs.push(approve(user, lendToken, _this.migratorAddress, DEFAULT_APPROVE_AMOUNT));
                }
              ...
              case 17:
                ...
                txCallback = _this.generateTxCallback({
                  rawTxMethod: function rawTxMethod() {
                ...
                txs.push({
                  ...
                  tx: txCallback,
                  gas: _this.generateTxPriceEstimation(txs, txCallback)
                });
                return _context.abrupt("return", txs);
...
function (_BaseService) {
  ...
  function () {
    ...
    /*#__PURE__*/
    runtime_1.mark(function _callee(_ref) {
      var user, amount, onBehalfOf, referralCode, convertedAmount, wethGatewayContract, txCallback;
      return runtime_1.wrap(function _callee$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              user = _ref.user, amount = _ref.amount, onBehalfOf = _ref.onBehalfOf, referralCode = _ref.referralCode;
              convertedAmount = parseNumber(amount, 18);
              wethGatewayContract = this.getContractInstance(this.wethGatewayAddress);
              txCallback = this.generateTxCallback({
                rawTxMethod: function rawTxMethod() {
                  return wethGatewayContract.populateTransaction.depositETH(onBehalfOf || user, referralCode || '0');
                },
                from: user,
                value: convertedAmount
              });
  ...
  function () {
    ...
    /*#__PURE__*/
    runtime_1.mark(function _callee2(_ref2) {
      var user, amount, debtTokenAddress, interestRateMode, referralCode, txs, convertedAmount, numericRateMode, delegationApproved, approveDelegationTx, wethGatewayContract, txCallback;
      return runtime_1.wrap(function _callee2$(_context2) {
        while (1) {
          switch (_context2.prev = _context2.next) {
            case 0:
              user = _ref2.user, amount = _ref2.amount, debtTokenAddress = _ref2.debtTokenAddress, interestRateMode = _ref2.interestRateMode, referralCode = _ref2.referralCode;
              txs = [];
              convertedAmount = parseNumber(amount, 18);
              numericRateMode = interestRateMode === InterestRate.Variable ? 2 : 1;
              _context2.next = 6;
            ...
            case 6:
              ...
              txCallback = this.generateTxCallback({
                rawTxMethod: function rawTxMethod() {
                  return wethGatewayContract.populateTransaction.borrowETH(convertedAmount, numericRateMode, referralCode || '0');
                },
                from: user
              });
              txs.push({
  ...
  function () {
    ...
    /*#__PURE__*/
    runtime_1.mark(function _callee3(_ref3) {
      var user, amount, onBehalfOf, aTokenAddress, txs, _this$erc20Service, isApproved, approve, convertedAmount, approved, approveTx, wethGatewayContract, txCallback;

      return runtime_1.wrap(function _callee3$(_context3) {
        while (1) {
          switch (_context3.prev = _context3.next) {
            case 0:
              user = _ref3.user, amount = _ref3.amount, onBehalfOf = _ref3.onBehalfOf, aTokenAddress = _ref3.aTokenAddress;
              txs = [];
            ...
            case 6:
              ...
              txCallback = this.generateTxCallback({
                rawTxMethod: function rawTxMethod() {
                  return wethGatewayContract.populateTransaction.withdrawETH(convertedAmount, onBehalfOf || user);
                },
                from: user
              });
              txs.push({
  ...
  function () {
    ...
    /*#__PURE__*/
    runtime_1.mark(function _callee4(_ref4) {
      var user, amount, interestRateMode, onBehalfOf, convertedAmount, numericRateMode, wethGatewayContract, txCallback;
      return runtime_1.wrap(function _callee4$(_context4) {
        while (1) {
          switch (_context4.prev = _context4.next) {
            case 0:
              user = _ref4.user, amount = _ref4.amount, interestRateMode = _ref4.interestRateMode, onBehalfOf = _ref4.onBehalfOf;
              convertedAmount = parseNumber(amount, 18);
              numericRateMode = interestRateMode === InterestRate.Variable ? 2 : 1;
              wethGatewayContract = this.getContractInstance(this.wethGatewayAddress);
              txCallback = this.generateTxCallback({
                rawTxMethod: function rawTxMethod() {
                  return wethGatewayContract.populateTransaction.repayETH(convertedAmount, numericRateMode, onBehalfOf || user);
                },
                from: user,
                value: convertedAmount
              });
...
__decorate([WETHValidator, __param(0, IsEthAddress('user')), __param(0, IsEthAddress('onBehalfOf')), __param(0, IsPositiveAmount('amount')), __metadata("design:type", Function), __metadata("design:paramtypes", [Object]), __metadata("design:returntype", Promise)], WETHGatewayService.prototype, "depositETH", null);

__decorate([WETHValidator, __param(0, IsEthAddress('user')), __param(0, IsPositiveAmount('amount')), __param(0, IsEthAddress('debtTokenAddress')), __metadata("design:type", Function), __metadata("design:paramtypes", [Object]), __metadata("design:returntype", Promise)], WETHGatewayService.prototype, "borrowETH", null);
...
__decorate([WETHValidator, __param(0, IsEthAddress('user')), __param(0, IsEthAddress('onBehalfOf')), __param(0, IsPositiveAmount('amount')), __metadata("design:type", Function), __metadata("design:paramtypes", [Object]), __metadata("design:returntype", Promise)], WETHGatewayService.prototype, "repayETH", null);
...
export { API_ETH_MOCK_ADDRESS, BigNumberZD, ChainId, DEFAULT_APPROVE_AMOUNT, DEFAULT_NULL_VALUE_ON_TX, ETH_DECIMALS, ExecutorType, GovernanceVote, HALF_RAY, HALF_WAD, InterestRate, LTV_PRECISION, MAX_UINT_AMOUNT, Market, Network, ProposalState, ProtocolAction, RAY, RAY_DECIMALS, SECONDS_PER_YEAR, SUPER_BIG_ALLOWANCE_NUMBER, SURPLUS, Stake, TxBuilder as TxBuilderV2, USD_DECIMALS, WAD, WAD_RAY_RATIO, aaveGovernanceV2Addresses, binomialApproximatedRayPow, calculateAvailableBorrowsETH$1 as calculateAvailableBorrowsETH, calculateAverageRate$1 as calculateAverageRate, calculateCompoundedInterest$1 as calculateCompoundedInterest, calculateHealthFactorFromBalances$1 as calculateHealthFactorFromBalances, calculateHealthFactorFromBalancesBigUnits$1 as calculateHealthFactorFromBalancesBigUnits, commonContractAddressBetweenMarketsV2, computeRawUserSummaryData$1 as computeRawUserSummaryData, computeUserReserveData$1 as computeUserReserveData, distinctContractAddressBetweenMarketsV2, distinctStakingAddressesBetweenTokens, eEthereumTxType, enabledNetworksByService, formatReserves$1 as formatReserves, formatUserSummaryData$1 as formatUserSummaryData, gasLimitRecommendations, getCompoundedBalance, getCompoundedStableBalance, getEthAndUsdBalance, normalize, pow10, rayDiv, rayMul, rayPow, rayToDecimal, rayToWad, uniswapEthAmount, index as v1, index$1 as v2, valueToBigNumber, valueToZDBigNumber, wadDiv, wadMul, wadToRay };
